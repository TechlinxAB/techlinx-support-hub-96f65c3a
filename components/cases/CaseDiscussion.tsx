...
import { Paperclip, SendHorizontal, Lock, RefreshCw, Trash2, FileText, X } from 'lucide-react';
...
import { getAttachmentUrl } from '@/utils/supabaseStorage';
...
const CaseDiscussion: React.FC<CaseDiscussionProps> = ({ caseId }) => {
  ...
  const handleAddReply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Attempt to save to server with attachments
      await addReply({
        caseId,
        userId: currentUser!.id,
        content: replyContent,
      ...
      storePendingSubmission('reply', {
        ...
        content: replyContent,
        isInternal: isInternalReply,
        // We can't store files in localStorage, so we only keep text content
      });
                        ...
                        {item.attachments && item.attachments.length > 0 && (
                          <div className="mt-2 pt-2 border-t border-gray-100">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {item.attachments.map((attachment: any, i: number) => {
                                const isFileOptimistic = attachment.isOptimistic;
                                const fileUrl = isFileOptimistic ? '#' : getAttachmentUrl(attachment.path);
                                ...
                                return (
                                  <a 
                                    ...
                                    className={`flex items-center gap-1 text-xs py-1 px-2 rounded-md bg-gray-50 border border-gray-200 ${isFileOptimistic ? 'opacity-60 pointer-events-none' : 'hover:bg-gray-100'}`}
                                  >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedFiles.map((file, index) => (
                      <div 
                        ...
                        className="flex items-center gap-1 bg-gray-100 text-sm py-1 px-2 rounded"
                      >
